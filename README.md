...
### `stage-08-polish`
**Concepts:** Small presentational components, composing UI, derived data for UI  
Extracts `PriorityBadge` from `TaskItem` as a reusable presentational component. `Sidebar` receives a `counts` object (derived data) and shows count badges per filter. App header shows the remaining active task count.
...
| 3:10 – 3:40 | `stage-08` | Polish: extracting components, task counts as derived data. |
